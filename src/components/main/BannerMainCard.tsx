...
const BannerMainCard = ({ type }: Props) => {
  return (
    <div
      className={clsx(
        'z-2 opacity-90 rounded-md bg-[#F1F5F9]',
        ' xl:w-205 md:w-192 flex flex-col items-center justify-center gap-10 px-16 py-10'
      )}
    >
      <h1 className={'font-semibold text-3xl text-slate-950'}>
        {type === 'link' ? '링크 농구교실' : '제이콕 배드민턴'}
      </h1>
      <div className='relative w-full xl:h-40 md:h-36'>
        <Image
